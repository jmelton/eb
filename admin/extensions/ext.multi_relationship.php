...
/***************************************************************************************************
****  Multi Relationship  **************************************************************************
        Select multiple entries to link to
...
class multi_relationship {
	var $settings		= array();
	
	var $name			= 'Multi Relationship';
	var $type			= 'mrel';
	var $version		= '1.0.6';
	var $description	= 'Allows a one to many relationship link';
	...
	function publish( $row, $field_data )
	{
		...
		if($row["field_type"] == $this->type)
		{
			...
			else
			{
				...
				$r .= $DSP->input_select_header('field_id_'.$row['field_id']."[]", 'multi', $row['field_ta_rows']);
				
				foreach ($relquery->result as $relrow)
				{
					$r .= $DSP->input_select_option($relrow['entry_id'], $relrow['title'], (in_array($relrow['entry_id'], $relentry_id)) ? 1 : 0);
				}
